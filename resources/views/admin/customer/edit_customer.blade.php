...
<div class="container py-2">
    <div class="mb-4">
        <div class="row place-userInfor">
            ...
            <form action="{{ route('admin.update.customer', ['id' => $customer->customerId]) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="col-lg-6">
                    ...
                    <div class="row">
                        <div class="form-group col-lg-12">
                            ...
                            <input type="text" class="form-control text-3 h-auto py-2" placeholder="Nhập tên khách hàng" name="tenkhachhang" value="{{ $customer->customerName }}" required="">
                        </div>
                        ...
                        <div class="form-group col-lg-6">
                            ...
                            <select class="form-select form-control h-auto py-2" id="customerType" data-msg-required="Chọn loại khách hàng" required="" name="loaikhachhang">
                                ...
                                @foreach($all_customer_type as $key => $customer_type)
                                    <option value="{{ $customer_type->customerTypeId }}" {{ $customer->customerTypeId == $customer_type->customerTypeId ? 'selected' : '' }}>
                        ...
                        <div class="form-group col-lg-6" id="genderField" style="display: {{ $customer->customerTypeId == 1 ? 'block' : 'none' }};">
                        ...
                        <div class="form-group col-lg-12">
                            ...
                            <input type="text" class="form-control text-3 h-auto py-2" placeholder="Nhập địa chỉ" name="diachi" value="{{ $customer->address }}" required="">
                        </div>

                        <div class="form-group col-lg-12">
                            <label class="form-label mb-1 text-2 required">Ghi chú</label>
                            <input type="text" class="form-control text-3 h-auto py-2" placeholder="Nhập ghi chú" name="ghichu" value="{{ $customer->note }}">
                        </div>

                        <div class="form-group col-lg-6">
                            <label class="form-label mb-1 text-2 required">Số điện thoại</label>
                            <input type="tel" class="form-control text-3 h-auto py-2" placeholder="Số điện thoại" name="sodienthoai" pattern="^0\d{9}$" maxlength="10" required="" value="{{ $customer->phoneNumber }}">
                            <small class="form-text text-muted">Nhập số điện thoại</small>
                        </div>

                        <div class="form-group col-lg-6">
                            <label class="form-label mb-1 text-2 required">Email</label>
                            <input type="email" class="form-control text-3 h-auto py-2" placeholder="Nhập email" name="email" value="{{ $customer->email }}" required="">
                        </div>
                    </div>
                </div>
